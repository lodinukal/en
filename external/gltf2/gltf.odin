...
parse :: proc(
	...
	if opt.is_glb {
		...
		json_header :=
			(cast(^GLB_Chunk_Header)(raw_data(
						file_content[content_index:content_index + GLB_CHUNK_HEADER_SIZE],
					)))
		if json_header.type != CHUNK_TYPE_JSON {
			return data, GLTF_Error {
				type = .Wrong_Chunk_Type,
				proc_name = #procedure,
				param = {name = "JSON Chunk"},
			}
		}
	...
	data.extensions_required = extensions_names_parse(
		parsed_object.(json.Object),
		EXTENSIONS_REQUIRED_KEY,
	)
	...
	for buf_idx := 0;
	    opt.is_glb && buf_idx < len(data.buffers) && int(content_index) < len(file_content);
	    buf_idx += 1 {
		chunk_header :=
			(cast(^GLB_Chunk_Header)(raw_data(
						file_content[content_index:content_index + GLB_CHUNK_HEADER_SIZE],
		...
		mem.copy(
			raw_data(data.buffers[buf_idx].uri.([]byte)),
			raw_data(file_content[content_index:]),
			int(chunk_header.length),
		)
...
unload :: proc(data: ^Data, allocator := context.allocator) {
	...
	context.allocator = allocator
	json.destroy_value(data.json_value, allocator)
...
warning_unexpected_data :: proc(proc_name, key: string, val: json.Value, idx := 0) {
	fmt.printf(
		"WARINING: Unexpected data in proc: %v at index: %v\nKey: %v, valalue: %v\n",
		proc_name,
		idx,
		key,
		val,
...
asset_parse :: proc(object: json.Object) -> (res: Asset, err: Error) {
	if ASSET_KEY not_in object {
		return res, GLTF_Error {
			type = .JSON_Missing_Section,
			proc_name = #procedure,
			param = {name = ASSET_KEY},
		}
	}
...
@(require_results)
animation_channel_target_parse :: proc(
	object: json.Object,
) -> (
	res: Animation_Channel_Target,
	err: Error,
) {
	path_set: bool
	...
	for material, idx in materials_array {
		...
		for k, v in material.(json.Object) {
			...
			case "occlusionTexture":
				res[idx].occlusion_texture = occlusion_texture_info_parse(
					v.(json.Object),
				) or_return

			case "pbrMetallicRoughness":
				res[idx].metallic_roughness = pbr_metallic_roughness_parse(
					v.(json.Object),
				) or_return
...
@(require_results)
normal_texture_info_parse :: proc(
	object: json.Object,
) -> (
	res: Material_Normal_Texture_Info,
	err: Error,
) {
	index_set: bool
	res.scale = 1
...
@(require_results)
occlusion_texture_info_parse :: proc(
	object: json.Object,
) -> (
	res: Material_Occlusion_Texture_Info,
	err: Error,
) {
	index_set: bool
	res.strength = 1
...
@(require_results)
pbr_metallic_roughness_parse :: proc(
	object: json.Object,
) -> (
	res: Material_Metallic_Roughness,
	err: Error,
) {
