...
get_debug_name :: proc(obj: ^d3d12.IObject) -> (name: string) {
	name_buffer: [win32.MAX_PATH]u16
	name_size: u32 = win32.MAX_PATH
	obj->GetPrivateData(
		d3d12.WKPDID_D3DDebugObjectNameW_UUID,
		&name_size,
		raw_data(name_buffer[:]),
	)
	len := win32.WideCharToMultiByte(
		win32.CP_UTF8,
		0,
		raw_data(name_buffer[:]),
		i32(name_size),
	...
	name_bytes := make([]u8, len, context.temp_allocator)
	...
	name = transmute(string)name_bytes
	return
}
...
create_d3d12_instance :: proc(
	...
	instance.create_compute_pipeline = create_compute_pipeline
	...
	if enable_graphics_api_validation {
		...
		if win32.SUCCEEDED(
			...
			debug_controller_1: ^d3d12.IDebug1
			if win32.SUCCEEDED(
				debug_controller->QueryInterface(
					d3d12.IDebug1_UUID,
					(^rawptr)(&debug_controller_1),
				),
			) {
				defer debug_controller_1->Release()
				debug_controller_1->SetEnableGPUBasedValidation(true)
			} else {
				log.errorf("Could not enable GPU based validation")
			}
		} else {
...
map_buffer :: proc(
	...
	if hr == dxgi.ERROR_DEVICE_REMOVED {
		device_remove_handler(b.device, false)
		return
...
create_device :: proc(
	...
	// hr := d3d12_instance.factory->EnumWarpAdapter(dxgi.IAdapter1_UUID, (^rawptr)(&device.adapter))
	// if !win32.SUCCEEDED(hr) {
	// 	error = .Unknown
	// 	return
	// }
...
cmd_set_descriptor_set :: proc(
	...
	for _, range_i in small_array.slice(&ds.ranges) {
		...
		// log.warnf("(%v) %v %v << 0x%x", u32(root_param + range_i), set_index, range_i, gpu_ptr.ptr)
		if b.is_graphics {
...
cmd_barrier :: proc(instance: ^Instance, cmd: ^Command_Buffer, #by_ptr desc: Barrier_Group_Desc) {
	...
	for barrier in desc.buffers {
		buf := (^D3D12_Buffer)(barrier.buffer)
...
resource_barrier :: proc(
	...
	// log.infof(
	// 	"Resource Barrier: %v -> %v, %v",
	// 	state_before,
	// 	state_after,
	// 	get_debug_name(resource),
	// )
...
create_descriptor_pool :: proc(
	...
	for num, type in heap_sizes {
		...
		// log.infof(
		// 	"cpu: 0x%x, gpu: 0x%x",
		// 	info.base_cpu_descriptor.ptr,
		// 	info.base_gpu_descriptor.ptr,
		// )
...
create_compute_pipeline :: proc(
	instance: ^Instance,
	device: ^Device,
	#by_ptr desc: Compute_Pipeline_Desc,
) -> (
	out_pipeline: ^Pipeline,
	error: Error,
) {
	...
	pso_desc.pRootSignature = layout.root_signature
	pso_desc.CS.pShaderBytecode = raw_data(desc.shader.bytecode[.DXIL])
	pso_desc.CS.BytecodeLength = len(desc.shader.bytecode[.DXIL])

	hr := d.device->CreateComputePipelineState(
		&pso_desc,
		d3d12.IPipelineState_UUID,
		(^rawptr)(&pipeline.pipeline),
	)
...
create_pipeline_layout :: proc(
	...
	if desc.enable_d3d12_draw_parameters_emulation {
		layout.indirect_sig = create_command_signature(
			...
			u32(size_of(Draw_Emulated_Desc)),
			true,
		)
		layout.indirect_indexed_sig = create_command_signature(
			...
			.DRAW_INDEXED,
			layout.root_signature,
			u32(size_of(Draw_Indexed_Emulated_Desc)),
			true,
		)
	}
